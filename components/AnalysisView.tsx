...
import { ShieldCheck, BrainCircuit, ScanLine, Terminal } from 'lucide-react';
...
export const AnalysisView: React.FC<AnalysisViewProps> = ({ fileName, fileUrl, fileType }) => {
  const [logs, setLogs] = useState<string[]>([]);
  const [progress, setProgress] = useState(0);
  const scrollRef = useRef<HTMLDivElement>(null);

  // Determine if it's a schedule run based on logs (hacky but effective for UI) or default to Est.
  // In a real app, pass 'mode' prop. Here we infer or randomize for effect if not specific.
  // For now, we assume if the filename implies schedule, or we switch to generic engineering logs.
  const activeLogs = SCHEDULING_LOGS; 

  const isImage = fileType?.startsWith('image/');
  
  // Use fileUrl if image, otherwise generic blueprint background
  ...
  useEffect(() => {
    ...
    // Logs
    const randomLogs = [...activeLogs].sort(() => Math.random() - 0.5); // Shuffle slightly for realism
  ...
  return (
    <div className="relative flex flex-col items-center justify-center h-full w-full bg-slate-900 overflow-hidden">
      ...
      <div className="relative z-20 w-full max-w-4xl p-6 flex flex-col items-center">
        ...
        <div className="mt-6 flex items-center space-x-3 bg-slate-800/50 backdrop-blur px-4 py-2 rounded-full border border-slate-700/50">
