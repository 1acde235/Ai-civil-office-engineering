...
const retryOperation = async <T>(
  ...
  try {
    ...
    console.warn(`Gemini API Attempt Failed (Retries left: ${retries}):`, error);
    ...
    const isClientError = status === 400 || message.includes("invalid argument") || message.includes("payload too large");
    const isRateLimit = status === 429 || message.includes("quota") || message.includes("resource exhausted") || message.includes("429");
    const isNetworkError = message.includes("xhr error") || 
    ...
    if (retries <= 0) {
        if (isRateLimit) throw new Error("System is busy (Google Quota Limit). Please wait 2 mins.");
        if (isNetworkError) throw new Error("Connection failed (Network/Timeout). Please check internet or reduce file size.");
        if (isClientError) throw new Error("Format Error: File too large or corrupted. Please upload a smaller file.");
        throw new Error(`AI Processing Failed: ${message.substring(0, 150)}...`);
    }
    ...
    const newDelay = isRateLimit ? 2000 : nextDelay * 1.5; 
    return retryOperation(operation, retries - 1, newDelay);
  }
};
...
// UPDATED: Highly detailed hierarchical structure as requested
const BUILDING_WBS = `
